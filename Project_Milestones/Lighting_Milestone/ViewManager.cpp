	...
	ShaderManager* pShaderManager)
{
	...
	g_pCamera->Position = glm::vec3(0.5f, 5.5f, 20.0f);
...
GLFWwindow* ViewManager::CreateDisplayWindow(const char* windowTitle)
{
	...
	// this callback is used to process mouse scroll wheel events
...
void ViewManager::Mouse_Scroll_Wheel_Callback(GLFWwindow* window, double xOffset, double yOffset)
{
	if (g_pCamera)
	{
		// Adjust the movement speed based on scroll direction
		g_pCamera->MovementSpeed += static_cast<float>(yOffset);
		// Clamp movement speed to a reasonable range
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// Process camera movement (zoom, pan, etc.)
	...
	// Switch to perspective view when 'P' is pressed
	if (glfwGetKey(m_pWindow, GLFW_KEY_P) == GLFW_PRESS)
	{
		bOrthographicProjection = false;
	}

	// Switch to orthographic view when 'O' is pressed
...
void ViewManager::PrepareSceneView()
{
	...
	// Define the projection matrix based on the current mode
	if (bOrthographicProjection)
	{
		...
		// Adjust the camera for orthographic projection
